...
const VerifyOtpForm = ({ register, isVerifyLoading }: any) => {
  return <div className="text-center space-y-6">
    ...
    <button type="submit" disabled={isVerifyLoading} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-bold">
      {isVerifyLoading ? <><LoadingSvg /> যাচাই হচ্ছে...</> : "যাচাই করুন (Verify)"}
    </button>
    <ResendOtp/>
  </div>
}
