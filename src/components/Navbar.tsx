...
export default function Navbar({ title, menus, user, isMainMenu = false }: Props) {
    ...
    <nav className="sticky top-0 z-50 shadow-sm font-sans">
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-3 flex justify-between items-center">
        <h1 className="font-black text-2xl flex items-center gap-2 text-slate-800">
          {!isMainMenu && <Link to={"/"} className="group-hover:-translate-x-1 transition-transform">←</Link>}
        ...
        {/* Desktop User Info */}
        <div onClick={() => navigate("/admin/profile")} className="cursor-pointer hidden md:flex items-center gap-3 bg-slate-50 px-4 py-1.5 rounded-2xl border border-slate-100">
      ...
      <div className={`
        ...
        ${isOpen ? "max-h-[500px] py-4 opacity-100" : "max-h-0 md:max-h-16 py-0 md:py-3 opacity-0 md:opacity-100 overflow-hidden"}
      `}>
        <div className="flex flex-col md:flex-row gap-6 md:items-center max-w-6xl mx-auto">
          ...
          <div onClick={() => navigate("/donor/profile")} className="cursor-pointer md:hidden pt-4 border-t border-slate-100 flex items-center gap-3">
